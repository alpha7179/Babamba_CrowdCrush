...
/// <summary>
/// 게임 종료 후 결과(Result) 및 요약(Summary) 화면의 UI 흐름과 연출을 관리하는 매니저
/// </summary>
/// <remarks>
/// <list type="bullet">
///   <item>DataManager 데이터 기반 별점 계산 및 펄스 애니메이션 재생</item>
///   <item>컨트롤러 입력(A버튼, 조이스틱)으로 페이지 전환 및 메인 이동</item>
///   <item>페이드·펄스 효과를 코루틴으로 처리하여 시각적 피드백 제공</item>
/// </list>
/// </remarks>
...
public class OuttroUIManager : MonoBehaviour
{
    ...
    [Tooltip("버튼 입력 활성화 여부")]
    [SerializeField] private bool _isButtonEnabled = false;
    [Tooltip("조이스틱 입력 활성화 여부")]
    [SerializeField] private bool _isJoystickEnabled = true;
    [Header("Scene Mode Settings")]
    [Tooltip("별도 씬으로 배치 시 true — Start()에서 자동 초기화. 오버레이 방식이면 false 유지")]
    ...
    private void Start()
    {
        // NOTE: 별도 씬 배치 시 true — 오버레이 방식(ShowOuttroUI 직접 호출)과 이중 초기화 방지
        if (_autoInitOnStart)
    ...
    /// <summary>
    /// 결과 패널을 표시하고 별점 애니메이션을 순차 재생하는 초기화 코루틴
    /// </summary>
    public IEnumerator InitializeRoutine()
    {
        ...
        for (int i = 0; i < starCount; i++)
        {
            if (i < starIcons.Length)
            {
                ...
                TriggerStarFeedback();
                yield return new WaitForSeconds(0.4f); // 별 간격 — 연출 리듬 유지
            }
        }
    }
    ...
    private int CalculateStarCount(int successCount, int mistakeCount, float playTime)
    {
        ...
        float timeLimitForMaxStar = 300f; // 5분 이내 완료 시 보너스 별
        float timeLimitForMinStar = 420f; // 7분 초과 시 별 차감
    ...
    public void NextPage() { if (currentPageIndex < summaryPages.Length - 1) { TriggerInteractionFeedback(); currentPageIndex++; UpdateSummaryPage(); } }
    public void PrevPage() { if (currentPageIndex > 0) { TriggerInteractionFeedback(); currentPageIndex--; UpdateSummaryPage(); } }
    ...
    {
        // NOTE: 진동 정규화 적용 — DataManager 설정값 기반 보정
